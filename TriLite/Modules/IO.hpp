 ...
 * format.
 */
Trimesh ReadMeshFile(const std::string& filepath);
 ...
 */
void WriteMeshFile(const Trimesh& mesh, const std::string& filepath,
                   bool binary_mode = true);

// Internal structures and functions
namespace {

constexpr int kAsciiDigitsPrecision = 10;
struct Vector3Hash {
...
std::vector<std::variant<V, Vector3d>> ConvertedToTrimeshPoints(
...
Trimesh ReadAsciiSTL(const std::string& filepath) {
...
Trimesh ReadBinarySTL(const std::string& filepath) {
...
Trimesh ReadSTL(const std::string& filepath) {
...
Trimesh ReadOBJ(const std::string& filepath) {
  std::ifstream file(filepath);
  std::string line;
...
Trimesh ReadOFF(const std::string& filepath) {
  std::ifstream file(filepath);
  std::string line;
...
Trimesh ReadBinaryPLY(const std::string& filepath) {
  std::ifstream file(filepath, std::ios::binary);
  std::string line;
...
Trimesh ReadAsciiPLY(const std::string& filepath) {
  std::ifstream file(filepath);
  std::string line;
  bool header_ended = false;
  int point_count = 0;
...
Trimesh ReadPLY(const std::string& filepath) {
  std::ifstream file(filepath, std::ios::binary);
  std::string line;
  getline(file, line);
  getline(file, line);
...
void WriteOBJ(const std::string& filepath, const Trimesh& mesh) {
  std::ofstream file(filepath);
  file << std::setprecision(kAsciiDigitsPrecision);
...
void WriteOFF(const std::string& filepath, const Trimesh& mesh) {
  std::ofstream file(filepath);
  file << std::setprecision(kAsciiDigitsPrecision);
...
void WriteSTL(const std::string& filepath, const Trimesh& mesh,
              bool binary_mode) {
  if (binary_mode) {
    ...
    auto write_float3 = [&file, &position_to_id](const Vector3d& vec,
      ...
      if (v != kInvalidId) {
        ...
        for (size_t i = 0; position_to_id.find(data) != position_to_id.end() &&
                           position_to_id[data] != v;
             ++i) {
          if (!std::isfinite(data[i % 3])) {
            data[i % 3] = -std::numeric_limits<float>::infinity();
          }
          data[i % 3] = std::nextafter(data[i % 3],
    ...
    double increment = pow(10, 1 - kAsciiDigitsPrecision);
    std::vector<Vector3d> positions{
    ...
    std::ofstream file(filepath);
    file << std::setprecision(kAsciiDigitsPrecision) << "solid mesh\n";
    for (F f : mesh.Faces()) {
...
void WritePLY(const std::string& filepath, const Trimesh& mesh,
              bool binary_mode) {
